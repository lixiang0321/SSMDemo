...
import www.ssm.util.bean.BaseUriUtil;
import www.ssm.util.bean.MessageUtil;
import www.ssm.util.validate.ActionValidationUtil;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

/**
 * 此验证类验证的是表单提交时候的数据配置的是否与，resource包
 * 下面配置的validation.properties文件的数据类型是否相同，
 * 如果不相同不继续向下执行action提交
 */
...
public class ValidationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        System.err.println("***************** request class = " + request.getClass());
